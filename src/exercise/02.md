...
Difference between useCallback and useMemo a'la Web DEv Simplified:
Use Case is the same, when we worry about the function's reference, we do not want to recreate it again (we have a theme for example). 
In case of useMemo, the value of the function will be returned.
In case of useCallback, the whole function is returned, so we can do some extra computations afterwards.
...
// So, function is recreated anyway, it is not just returned if the dep array does not change??
// Memoization comes in handy, when we need to recalculate (or rerender) the same values (or functions, components) multiple times. For simple functions, we can pass prevValues to the functions. 
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=02%3A%20useCallback%3A%20custom%20hooks&em=recskova%40sodefa.de).
